...
- **OpenAI integration**: `src/controllers/openai.js` (large file) handles both comment-highlight generation and natural-language search query parsing (`parseNaturalLanguageQuery`, used by `locations.find`). The NL path has an in-memory cache, a fixed-window global rate limit, a concurrency cap, and a timeout; on any failure it degrades to using the raw query as a plain search string.
